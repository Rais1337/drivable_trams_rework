...
function ENT:PostEntityPaste(ply,ent,createdEntities)
	...
	if self.BogeyType == "tatra" then
		self:SetModel("models/props_vehicles/trams/tatra_t3/tatra_bogey.mdl")
	end
	if IsValid(self.Wheels) then
		if self.BogeyType == "tatra" then
...
function ENT:InitializeWheels()
	...
	if not IsValid(self.Wheels) then
		--print(1)
		wheels = ents.Create("gmod_trams_wheels")
		if self.BogeyType == "tatra" then
...
function ENT:Think()
	...
	motorPower = math.max(-2.0,motorPower)
	...
	if IsValid(ent) and ent:GetNWEntity("seat_driver") and IsValid(ent:GetNWEntity("seat_driver")) then 
		local ply = ent:GetNWEntity("seat_driver"):GetDriver()
		if IsValid(ply) and ply:KeyDown( IN_SPEED ) then
			if !self:GetNWBool("horn") then
				self:SetNWBool("horn", true)
			end
    	else
			if self:GetNWBool("horn") then
				self:SetNWBool("horn", false)
			end
    	end
    	if IsValid(ply) and ply:KeyDown( IN_WALK ) then
			if !self.KeyPressedValdis then
				if !self.ToggledValdis then
					if !self:GetNWBool("valdis") then
						self:SetNWBool("valdis", true)
					end
					self.ToggledValdis = true
				else
					if self:GetNWBool("valdis") then
						self:SetNWBool("valdis", false)
					end
					self.ToggledValdis = false
				end
				self.KeyPressedValdis = true
			end
    	else
			/*if self:GetNWBool("valdis") then
				self:SetNWBool("valdis", false)
...
function ENT:SpawnFunction(ply, tr)
	...
	if not inhibitrerail then Drivtrams.RerailBogey(ent) end
	return ent
end