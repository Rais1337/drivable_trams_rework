...
local function onDispatcherMessage(msg)
	if string.sub(msg,1,1) == "@" then
		...
		if cmd[1] == "red" then
			for k,v in pairs(Drivtrams.TrafficLights) do
				if v:IsValid() and (v.EquipmentID == tonumber(cmd[2])) then
					v.TelemetryOverrideToRed = true
					if Drivtrams.TrafficLightStates[v] then
						Drivtrams.TrafficLightStates[v].state = -1
					end
				end
			end
		...
		Drivtrams.DriverChatBacklog["global"] =
			Drivtrams.DriverChatBacklog["global"] or {}
			
		table.insert(Drivtrams.DriverChatBacklog["global"],"Dispatcher: "..msg)
	end
end
...
local s
local http_data
function Drivtrams.HTTPRequest(request)
...
--	s:send("GET /metrostroi/ms-act.php HTTP/1.1\n")
	...
	-- Create timer
	timer.Create("Drivtrams_HTTPRequest",0.01,0,function()
		...
		if status and (status ~= "timeout") then
			...
			timer.Destroy("Drivtrams_HTTPRequest")
			onSuccess(content)
		end
	end)
end
...
Drivtrams.TrafficLightStates = {}
Drivtrams.SwitchStates = {}
function Drivtrams.UpdateTelemetry()
	if true then return end
	if not Drivtrams.TrainPositions then return end
	
	local request = "query=update&r={ \"trains\": {"
	local f = false
	for train,data in pairs(Drivtrams.TrainPositions) do
...
--			print(train.WagonID)
	...
	local i = 1
	local f = false
	for train,data in pairs(Drivtrams.DriverChatBacklog) do
		if (train ~= "global") and train:IsValid() then
			...
			Drivtrams.DriverChatBacklog[train] = {}
		else
			for k,v in pairs(data) do
				if f == true then request = request.."," end
				f = true
	...
	local f = false
	for light,data in pairs(Drivtrams.TrafficLightPositions) do
		if light:IsValid() and light.EquipmentID then
			...
			if (not Drivtrams.TrafficLightStates[light]) or
				 (Drivtrams.TrafficLightStates[light].state ~= bits) or
				 (CurTime() - Drivtrams.TrafficLightStates[light].time > 120) then
				if f == true then request = request.."," end
				f = true
				Drivtrams.TrafficLightStates[light] = {
					state = bits,
					time = CurTime(),
				}
	...
	request = request.."}}"
	Drivtrams.HTTPRequest(request)
--	print(request)
--	http.Post("http://foxworks.wireos.com/metrostroi/ms-act.php",{ r = request, query = "update" },onSuccess,onFailure)
--	print("POST",CurTime())
end

timer.Create("Drivtrams_UpdateTelemetry",1.0,0,Drivtrams.UpdateTelemetry)
...
-- Get driver chats
--------------------------------------------------------------------------------
if not Drivtrams.DriverChatBacklog then
	Drivtrams.DriverChatBacklog = {}
end
